...
c
      ...
      real k0,ksqw,omega,rmax,zmplt,dr,dz
      integer lz, nz
c
...
c
      ...
      open(unit=2,status='unknown',file='imag.grid',form='unformatted')
      open(unit=3,status='unknown',file='real.grid',form='unformatted')
      open(unit=4,status='unknown',file='ssp.grid',form='unformatted')
      open(unit=5,status='unknown',file='rho.grid',form='unformatted')
      open(unit=6,status='unknown',file='parameters.txt')
c
...
c
     ...
     >   alpb(mz),f1(mz),f2(mz),f3(mz),ksqw(mz),tlg(mz),ssp(mz),
     >   rho(mz),omega
      integer lz
c
      read(1,*)
...
c
    ...
    6 continue
      ...
      ii=ii+1
    7 continue
      write(4)(ssp(j),j=1,lz)
      write(5)(rho(j),j=1,lz)
c
...
c
     ...
     >   f3,r1,r2,r3,s1,s2,s3,pd1,pd2,lz)
     ...
     >   f1(mz),f2(mz),f3(mz),alpw(mz),alpb(mz),ssp(mz),rho(mz)
      integer lz
c
...
c
     ...
     > ,dr,v)
      complex ur,u(mz),v(mz)
      real f3(mz),rep(mz),imp(mz),tlg(mz)
c
c      open(unit=2,status='unknown',file='p_imag.grid',form='unformatted')
c      open(unit=3,status='unknown',file='p_real.grid',form='unformatted')
c      open(unit=4,status='unknown',file='k.grid',form='unformatted')
c      open(unit=5,status='unknown',file='rho.grid',form='unformatted')
      
c      tl=-20.0*alog10(cabs(ur)+eps)+10.0*alog10(r+eps)
c
      mdr=mdr+1
      if(mdr.eq.ndr)then
      mdr=0
...
c      tlg(j)=-20.0*alog10(cabs(ur)+eps)+10.0*alog10(r+eps)
      ...
      imp(j)=AIMAG(ur) 
    1 continue
      write(2)(rep(j),j=1,lz)
      write(3)(imp(j),j=1,lz)
